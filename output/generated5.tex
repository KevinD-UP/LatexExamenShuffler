...
v_1 = [ m\mapsto 1] \quad
v_2= [ m\mapsto 1, g\mapsto 1 ]  \quad
v_3= [ g\mapsto 1, z\mapsto 1 ]$.
Remplissez le tableau suivant ($0$ ou $1$ dans les trois premières colonnes et $vrai$ ou $\mathit{faum}$ dans le reste)~:
\begin{tabular}{|c|c|c|c|c|c|c|}
  \hline
    Formule $p$ &$\inte{p}{v_1}$&$\inte{p}{v_2}$&$\inte{p}{v_3}$&$p$ valide ?&$p$ satisfaisable ?&$p$ contradictoire ?\\[0.5ex]
    \hline
    $((\neg m \vee g) \land m)$&&&&&&\\[0.5ex]
    \hline
    $(\neg g \land (z \land (m \land g)))$&&&&&&\\[0.5ex]
    \hline
    $((m \land z) \vee ((\neg m \land z) \vee \neg z))$&&&&&&\\[0.5ex]
    \hline
  \end{tabular}
